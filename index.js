...
async function createBlogSummary() {
  ...
  if (!blogs || blogs.length === 0) {
    ...
    blogs.forEach((blog) => {
      ...
      numLikesp.textContent = blog.likes.length.toString();
      ...
      const blh3 = document.createElement("h4");
      blh3.className = "blog-sum-main-title";
      blh3.textContent = blog.title;
      titleDiv.append(blh3);

      const pdateTime = document.createElement("p");
      pdateTime.textContent =
      ...
      pblgSummury.innerHTML = blog.description.substr(0, 100) + "...";
      summuryBlog.append(pblgSummury);

      const linkBlog = document.createElement("a");
      linkBlog.href = `./blogOne.html?:${blog._id}`;
