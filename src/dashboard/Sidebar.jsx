...
const ICONS = {
  ...
  accounts: <svg width="18" height="18" viewBox="0 0 18 18"><rect x="1.5" y="4" width="15" height="10.5" rx="2" fill="none" stroke="currentColor" strokeWidth="1.7" /><path d="M1.5 7.5h15" stroke="currentColor" strokeWidth="1.7" /><rect x="11.5" y="9.6" width="3.5" height="2.4" rx="1.2" fill="currentColor" /></svg>,
  assets: <svg width="18" height="18" viewBox="0 0 18 18"><path d="M9 1.8l6.5 4v6.4l-6.5 4-6.5-4V5.8z" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" /><path d="M9 1.8v6M9 8l6.5-2.2M9 8l-6.5-2.2" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" /></svg>,
...
export default function Sidebar({ tab, setTab, open, onClose, derived, counts, debt }) {
  ...
  return (
    <aside className="dash-sidebar" data-open={open ? 'true' : 'false'} style={{ width: 256, flexShrink: 0, background: 'var(--sidebar)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', padding: '22px 16px', position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
      ...
      <NavItem id="accounts" label="Accounts" active={tab === 'accounts'} onClick={() => setTab('accounts')} />
      <NavItem id="assets" label="Assets" active={tab === 'assets'} onClick={() => setTab('assets')} />
